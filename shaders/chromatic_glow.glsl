// Glow effect in perceptual (Oklab) color space
// Based on kalgynirae's Ghostty passable glow shader
// https://github.com/kalgynirae/dotfiles/blob/main/ghostty/glow.glsl

// === CONFIGURATION ===
const float GLOW_DIM_CUTOFF = 0.35;      // Lightness below which glow is applied
const float GLOW_BRIGHT_CUTOFF = 0.65;   // Lightness above which glow is stronger
const float GLOW_LIGHTNESS_DIM = 0.05;   // Glow lightness weight for dim pixels
const float GLOW_LIGHTNESS_BRIGHT = 0.10;// Glow lightness weight for bright pixels
const float GLOW_CHROMA_WEIGHT = 0.3;    // Glow color bleed weight
const float BRIGHT_BOOST = 1.2;          // Lightness multiplier for bright pixels
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (source.x > GLOW_DIM_CUTOFF) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (c.x > GLOW_DIM_CUTOFF) {
                glow.yz += c.yz * weight * GLOW_CHROMA_WEIGHT;
                if (c.x <= GLOW_BRIGHT_CUTOFF) {
                    glow.x += c.x * weight * GLOW_LIGHTNESS_DIM;
                } else {
                    glow.x += c.x * weight * GLOW_LIGHTNESS_BRIGHT;
                }
            }
        }
